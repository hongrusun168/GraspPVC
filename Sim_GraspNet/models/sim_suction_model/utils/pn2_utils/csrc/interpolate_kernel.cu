...
std::vector<at::Tensor> PointSearch(
  ...
  auto index = at::zeros({batch_size, num_query, num_neighbours}, query_xyz.options().dtype(at::kLong));
  auto distance = at::zeros({batch_size, num_query, num_neighbours}, query_xyz.options());
  
  const auto block = std::min((uint64_t)num_key, MAX_THREADS);

  AT_DISPATCH_FLOATING_TYPES(query_xyz.scalar_type(), "PointSearch", ([&] {
...
at::Tensor InterpolateForward(
  ...
  auto output = at::zeros({batch_size, channels, num_select}, input.options());
...
at::Tensor InterpolateBackward(
  ...
  auto grad_input = at::zeros({batch_size, channels, num_inst}, grad_output.options());
  CHECK_EQ(index.size(0), batch_size);
  CHECK_EQ(index.size(2), K);
  CHECK_EQ(weight.size(0), batch_size);
  CHECK_EQ(weight.size(1), num_select);
