...
std::vector<at::Tensor> BallQuery(
  ...
  auto index = at::zeros({batch_size, num_centroids, num_neighbours}, points.options().dtype(at::kLong));
  index.set_requires_grad(false);
  auto count = at::zeros({batch_size, num_centroids}, index.options());
  CHECK_CUDA(index); CHECK_CONTIGUOUS(index);
  CHECK_CUDA(count); CHECK_CONTIGUOUS(count);

  const auto block = get_block(num_centroids);

  AT_DISPATCH_FLOATING_TYPES(points.scalar_type(), "BallQuery", ([&] {
