...
void knn_device(float* ref_dev, int ref_nb, float* query_dev, int query_nb,
  ...
  unsigned int size_of_int64  = sizeof(int64_t);

  float* dist_host = new float[query_nb * k];
  int64_t*  idx_host  = new int64_t[query_nb * k];
  ...
  cudaMemcpy(&idx_host[0], ind_dev,
      query_nb * k * size_of_int64, cudaMemcpyDeviceToHost);

  int i = 0;
  for(i = 0; i < 100; i++){
    printf("IDX[%d]: %lld\n", i, (long long)idx_host[i]);
  }
#endif
}
