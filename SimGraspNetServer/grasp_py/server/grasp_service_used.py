...
class RealTimeGraspDetector:
    ...
    def perform_grasp_detect_PVC(self, context = None, stop_event = None ):                                             # 执行 PVC 管的抓取任务
        # try:
            # if self._PVC_processing == True:
            #     print("[INFO]: PVC, 已经在执行抓取检测流程了, 请稍等 ...")
            #     return False, "正在执行抓取检测流程, 请稍等"
            # if self._EVA_processing == True:
            #     print("[INFO]: PVC, EVA 管正在执行抓取检测流程, 请稍等 ...")
            #     return False, "EVA 管正在执行抓取检测流程, 请稍等"
    ...
    def perform_grasp_detect_EVA(self,context = None, stop_event = None):                                                             # 执行 EVA 管的抓取任务
        # try:
    ...
    def _Is_cancelled(self, context = None):
...
def grasp_pvc_and_eva(context):
    ...
    success = 0
    message = ""
    success, message = grasp_detector.perform_grasp_detect_PVC(context, stop_event)
    if success == False:
        return 1, "提前结束"
    success, message = grasp_detector.perform_grasp_detect_EVA(context, stop_event)
