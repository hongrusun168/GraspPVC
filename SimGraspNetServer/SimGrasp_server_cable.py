    ...
    def Capture(self, which_side = None, save = False):
        ...
        if save == True:
            ...
            if which_side == "PVC":
                ...
                img_path = os.path.join(save_dir, "EVA", "images", f"{timestamp}.png")
                depth_npy_path = os.path.join(save_dir, "EVA", "depth_npys", f"{timestamp}.npy")
                depth_png_path = os.path.join(save_dir, "EVA", "depth_imgs", f"{timestamp}.png")
...
class RealTimeGraspDetector:
    ...
    def perform_grasp_detect_PVC(self):                                                             # 执行 PVC 管的抓取任务
        # try:
            ...
            time.sleep(1.25 * 3)
            self._Gripper.set_pos(val = 80)                                                         # 调整夹爪到合适的宽度,以防止与 PVC 内管发生碰撞
            end_time = time.time()
            ...
            filter_pointcloud_by_xy(scene_pcd, x_range = (-0.255000, 0.090000), y_range = (-0.973493, -0.550379), z_range = (-0.1, 0.30))
            filter_pointcloud_by_xy(PVC_pcd, z_range = (-0.1, 0.30))
            ...
            gg_array = self.generate_grasps_from_pointcloud(PVC_pcd)
            ...
            best_gg_grasp2 = translate_grasp_point_along_direction(best_gg, distance = self._PVC_pose_distance - 0.005 + best_gg[1] * 0.01)
    ...
    def close_gripper(self, pose = None):
        self._Gripper.set_pos(val = pose)   


    def generate_grasps_from_pointcloud(self, pcd, n_samples=1000, n_rotations=20):
        """
        从点云随机采样指定数量的点，每个采样点生成多个绕法线旋转的抓取姿态
        
        Args:
            points: (N, 3) 点云数组
            n_samples: 采样的点云数量（默认1000）
            n_rotations: 每个点绕法线旋转的等分数（默认20，即每9度一个姿态）
        
        Returns:
            grasps: (n_samples * n_rotations, 17) 抓取姿态数组
        """
        points = pcd.points
        ...
        centers = points[sampled_indices]
        
        # 抓取朝向向上：接近方向为 (0, 0, 1)
        approach = np.array([0.0, 0.0, 1.0], dtype=np.float32)
        
        # 生成旋转角度（0到180度，不包括180度以避免重复）
        angles = np.linspace(0, np.pi, n_rotations, endpoint=False)
        ...
        for i in range(n_samples):
            ...
            # 为每个点生成n_rotations个不同角度的抓取姿态
            for theta in angles:
                cos_theta = np.cos(theta)
                sin_theta = np.sin(theta)
                
                # 绕z轴旋转的x轴和y轴（水平面内的旋转）
                x_axis = np.array([cos_theta, sin_theta, 0.0], dtype=np.float32)
                y_axis = np.array([-sin_theta, cos_theta, 0.0], dtype=np.float32)
                
                # 构建旋转矩阵（3x3），z轴为向上方向
                rotation = np.column_stack([x_axis, y_axis, approach])
                
                # 组装17维向量
                grasp = np.zeros(17, dtype=np.float32)
                ...
                # 第5-13维：旋转矩阵，展平为9个值（行优先）
                grasp[4:13] = rotation.flatten()
    ...
    def perform_grasp_detect_EVA(self):                                                             # 执行 EVA 管的抓取任务
        try:
            self._EVA_processing = True                                                             # 标定为正在执行
            ...
            filter_pointcloud_by_xy(scene_pcd, x_range = (0.189594, 0.539492), y_range = (-0.971354, -0.553787),  z_range = (-0.1, 0.3))
            filter_pointcloud_by_xy(EVA_pcd, z_range = (-0.1, 0.3))
...
if __name__ == "__main__":
    grasp_detector.close_gripper(pose = 0)
    ...
    for i in range(1):
        start_time = time.time()
        # grasp_detector.perform_grasp_detect_EVA()
